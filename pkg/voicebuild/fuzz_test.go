...
// FuzzAssembleWithKeygroups exercises the sector layout math on random but
// well-shaped voice slices, bounded to small inputs (≤8 voices, ≤4 KB audio
// each). It asserts no panic, and sector alignment on any output produced.
func FuzzAssembleWithKeygroups(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, nVoices uint8, samplesPerVoice uint16, rateMask uint8) {
		...
		for i := range n {
			// Each voice is one sector header plus samples*2 bytes of audio,
			// stamped with a per-voice sentinel so cross-voice shuffling in
			// the sector packing shows up after assembly.
		...
		if err != nil {
			// Errors are acceptable; only a panic fails the fuzz.
			return
		}
