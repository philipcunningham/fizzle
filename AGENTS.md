...
- `web/app/` is the React and TypeScript front end. `src/boundary/contract.ts` is the typed boundary both sides agree on. `src/core/worker.ts` runs the core in a Web Worker and serialises calls onto it. Component tests use strict, explicitly staged protocol stubs rather than a second editor implementation. `src/shell/` owns view workflows; screens, controls, and dialogs live in `src/screens/`, `src/ui/`, and `src/dialogs/`. The front end owns view state only; no FZ format logic lives outside the core.
...
- `pkg/fzutil/` contains bounded file reads, resampling, voice-name normalisation, and the private byte-walk used by `pkg/fzf` constructors. Application code consumes canonical `pkg/fzf` documents and views rather than raw layout counters.
