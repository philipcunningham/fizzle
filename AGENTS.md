...
- `pkg/studio/` contains the interactive terminal UI (`fizzle studio`). Sub-packages: `app/` (root tview.Application + key dispatch + mouse dispatch with focus-aware flush + modal stack wiring), `model/` (in-memory FZF state with undo/redo and dirty tracking), `widgets/` (voicelist, voicedetails, banktab, loopdetails, globaleffect, statusline; each widget exposes `InputFields()` so the app can enumerate the focusable text inputs for flush-on-Ctrl-S and flush-on-mouse-click-elsewhere), `modal/` (k9s-style Pages stack), `helpers/` (note-name parser, accept-function factories, output-bitmask formatter, key-action registry).
