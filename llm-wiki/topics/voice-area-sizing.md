...
The vn is trusted in one direction only: where it runs above the walk. `testdata/synthetic/TECHNO.img` carries a DIS vn of 30 while the walk finds 32 live, named voices. A vn at or below the walk is therefore an undercount and the walk wins. Which reading the real firmware makes of such a disk is untested.

A standalone `.fzf` export can carry the count in a fizzle-defined marker: the magic `fz` plus vn at bank offset 0x294. That sits inside the same firmware padding region as the 0x290 wave total. Readers honour it only when the magic matches and the count validates above the walk. The offset holds firmware garbage on 203 of 235 corpus dumps, so a bare count there means nothing.
