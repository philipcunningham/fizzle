...
- Type sizes follow HP 64000 cross-compiler conventions, not modern C. `long` is 4 bytes, `int` is 2, and `short` is 1. Reading `struct voicedata` with 2-byte shorts shifts every offset past 0x12. The spec's offset comments are authoritative.
...
- The `loopst` struct comment assigns four bits to loop fine. The spec prose assigns eight bits with values from 0 to 255. The owner's manual defines EX FINE as 1/256 of a sample, and corpus round trips use the full byte. The comment is therefore wrong.
