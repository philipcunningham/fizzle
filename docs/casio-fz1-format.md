...
**`bstep` is per-bank and counts key splits, not file-level voices.** Spec section 2-2 gives `bstep` as "the current number of key splits or the number of voices which the bank uses". For most files in the wild the two coincide: every key split uses a distinct voice, so `bstep` from bank 0 equals the file's `vn`. Multi-voice-per-key-split kits (factory `Drums.fzf` on FL-4, `Percussion.fzf` on FL-6, and a similar pattern across ~15 community shareware files) declare `bstep > vn`: the bank uses `bstep` key splits backed by `vp[]` references into a smaller set of distinct voices.

Parsers must size the voice area by `vn` (from the file head) when available, or by walking and validating slots when the file head is absent (the standalone-`.fzf` case after `disk get`). Sizing by `bstep` works for the simple case but reads garbage for the legitimately complex one. Confirmed against the FZ-1 Factory Library: 226 of 227 community files have `bstep == vn`; the remaining ~15 use the legitimate split-sharing pattern.
