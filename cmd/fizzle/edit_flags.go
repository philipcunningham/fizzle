...
func collectLFOPatches(cmd *cli.Command, params *fzvinfo.VoiceParams) ([]voiceedit.Edit, error) {
	...
	// The whole lfo_name byte, because both halves matter. Setting the
	// waveform keeps bit 7's phase sync, and setting the sync keeps the
	// waveform index. Reconstructing the byte from one half alone
	...
	// since nothing above wrote the byte. The builder's own patch
	// carries the offset and size, so it goes through unaltered.
	if !cmd.IsSet("lfo-wave") {
