...
import type { Channel, Core, CoreResult, InstrumentVoice, Snapshot } from "../boundary/contract";
...
import { readBytes, relativePath, saveFile } from "./fileio";
import { useDocumentSession } from "./useDocumentSession";
import { useAuditionData } from "./useAuditionData";
...
export function EditorShell({ core }: { core: Core }) {
  ...
  const {
    ...
    highlight,
    focusRoot,
  } = useEditorSelection(instrument);

  const { peaks, auditionData, slotPCM } = useAuditionData(
    core,
    tab,
    instrument,
    bank,
  ...
  const { estimate, estimateError } = useImportEstimate(
    core,
    dialog,
    snap?.revision ?? 0,
    rate,
