...
const browser = await chromium.launch({
  channel: "chrome",
  // Subpixel text antialiasing and hinting are decided per run from the
  // machine's state, which moved a few dozen pixels of the chrome
  // between two runs of the same script. Off, a rerun on one machine
...
const shot = async (name, selector) => {
  ...
  console.log(`wrote ${name}.png (${String(buf.length)} bytes)`);
};
